...
import { SaleorAdapter } from "./adapter";
...
function buildHandler() {
  ...
  const sl = new SaleorClient(config.saleor);
  const adapter = new SaleorAdapter({ saleor: sl, siteUrl: config.siteUrl });

  const exactRoutes: Record<string, RouteHandler> = {
    ...buildDiscoveryRoutes(config),
    "/healthz": buildHealthRoute(sl, VERSION),
...
async function main() {
  ...
  server.listen(config.port, config.host, () => {
    ...
    console.log(
      `agentic-commerce-for-saleor v${VERSION} listening on http://${config.host}:${config.port}`,
    );
    // eslint-disable-next-line no-console
    console.log(`  merchant slug:       ${config.merchantSlug}`);
    // eslint-disable-next-line no-console
    console.log(`  saleor api:        ${config.saleor.apiUrl}`);
    // eslint-disable-next-line no-console
