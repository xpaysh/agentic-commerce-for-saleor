...
const sessions = createSessionStore({
  pluginId: process.env.ACP_SESSION_PLUGIN_ID || "agentic-commerce-for-saleor",
});
...
export function buildAcpRouteTable(adapter: SaleorAdapter): RouteTable<RouteHandler> {
...
function createSession(adapter: SaleorAdapter): RouteHandler {
...
function updateSession(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getSession(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return acpError(400, "invalid_request", "checkout_session_id required");
    const s = await sessions.get(id) as AcpSession | null;
...
function completeSession(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return acpError(400, "invalid_request", "checkout_session_id required");
    const s = await sessions.get(id) as AcpSession | null;
...
function getOrderRoute(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
