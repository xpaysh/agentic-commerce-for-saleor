...
export function buildAp2RouteTable(adapter: SaleorAdapter, merchantSlug: string): RouteTable<RouteHandler> {
...
function checkoutWithMandate(adapter: SaleorAdapter, merchantSlug: string): RouteHandler {
  return async (req): Promise<RouteResponse> => {
