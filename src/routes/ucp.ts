...
import type { SaleorAdapter } from "../adapter";
import type {
  Cart,
  Order,
  Product,
...
export function buildUcpRouteTable(adapter: SaleorAdapter): RouteTable<RouteHandler> {
...
function catalogSearch(adapter: SaleorAdapter): RouteHandler {
...
function catalogLookup(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function createCart(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getCart(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
...
function updateCart(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
    if (!id) return bad(400, "missing_id", "cart id required in path");
...
function checkout(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
...
function getOrder(adapter: SaleorAdapter): RouteHandler {
  return async (req): Promise<RouteResponse> => {
    const id = req.params?.id;
